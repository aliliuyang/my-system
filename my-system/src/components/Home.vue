  ...
  <el-row class="container">
    ...
    <el-col :span="24" class="topbar-wrap">
      <div class="topbar-logo topbar-btn">
        <a href="/"><img src="../assets/logo.jpg" style="width:42px;margin-top: 5px;"></a>
      </div>
      ...
      <div class="topbar-account topbar-btn">
        <el-dropdown trigger="click">
          <span class="el-dropdown-link userinfo-inner">
            ...
            {{sysUserName}}  <i class="iconfont icon-down"></i></span>
    ...
    <el-col :span="24" class="main">
      ...
      <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
        ...
        <!--菜单展开时的显示情况-->
        <el-menu v-show="!collapsed" default-active="0" :class="collapsed?'bz':'bk'" router>
          <template v-for="(item,index) in $router.options.routes" v-if="item.menuShow">
            <el-submenu v-if="!item.leaf" :index="index+''">
              <template slot="title">
                <i :class="item.iconCls"></i>
                <span>{{item.name}}1231</span>

              </template>
              <el-menu-item v-for="term in item.children" :key="term.path" :index="term.path" v-if="term.menuShow">
                <span>{{term.name}}4546</span>
              </el-menu-item>
            </el-submenu>
            <el-menu-item v-else-if="item.leaf&&item.children&&item.children.length" :index="item.children[0].path" class="el-submenu__title">
              <i :class="item.iconCls"></i><span>{{item.children[0].name}}7878</span>
            </el-menu-item>
          </template>
        </el-menu>
